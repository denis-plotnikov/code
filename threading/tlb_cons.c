...
{
	start_flags[thread_num * CACHE_LINE_SIZE] = (char) 1;
}

void drop_flag(int thread_num)
{
	start_flags[thread_num * CACHE_LINE_SIZE] = (char) 0;
}
...
static void *director(void *thr_info)
{
	...
	while (true) {
		...
		if (counter == 0) {
			...
			// as mapping checked - exchange values to be checked
			int tmp = mapping[0].val;
			mapping[0].val = mapping[1].val;
			mapping[1].val = tmp;
			//printf("Mapping has been changed\n");
			runs_done++;
